# staff-system-detection Specification

## Purpose

Detect grand-staff systems on rendered score pages and expose them through an API so the
reader can scroll in a music-aware way. Detection deskews the page, reads a horizontal
projection profile, groups staff lines into staves and staves into systems by their
connectors, and reports system bounding boxes in full-page canvas coordinates.
